...
window.onload = async () => {
  ...
  try {
    ...
    const { vertex: worldVert, fragment: worldFrag } = await fetchShaders(
      "shaders/world-vert.glsl",
      "shaders/world-frag.glsl"
    );
    worldProg = twgl.createProgramInfo(gl, [worldVert, worldFrag]);

    let { vertex: spriteVert, fragment: spriteFrag } = await fetchShaders(
  ...
  for (let instance of instances) {
    ...
    if (instance.animTime > instance.object.animSpeed) {
      ...
      instance.textureIndex =
        (instance.textureIndex + 1) % instance.object.textures.length;
...
function handleInputs(deltaTime) {
  ...
  if (inputMap["w"] || inputMap["ArrowUp"]) {
    playerBody.velocity.set(
      -playerFacing[0] * moveSpeed,
      -playerFacing[1] * moveSpeed,
      -playerFacing[2] * moveSpeed
    );
  }
  ...
  if (inputMap["q"] || inputMap["z"]) {
    playerBody.velocity.set(
      (-playerFacing[2] * moveSpeed) / 2,
      0,
      (playerFacing[0] * moveSpeed) / 2
    );
  }
